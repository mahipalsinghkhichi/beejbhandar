...
const OwnerReport = () => {
  ...
  const { Loadstate, Loadcity, Loadarea, Loadimage, Ownervar } = useSelector(state => state.cartreducer);
// console.log("Ownervar,,,,,,,,,,,,,,,,,,",Ownervar);
  ...
  if (state.search) {
    tempAppUsersArray = tempAppUsersArray.filter(name => (name.value.shope_type).match(new RegExp(state.search, "i")) ||
  ...
  return (
    ...
    <AdminDashboard/>
      ...
      <h2 className=" text-center  text-light"> Owner Report</h2>
      </div>
